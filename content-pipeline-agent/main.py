...
class ContentPipelineFlow(Flow[ContentPipelineState]):
    ...
    def handle_make_blog(self):
        ...
        if blog_post is None:
            result = llm.call(
                f"""
                Make a blog post on the topic with SEO practices {self.state.topic} using the following research:
    ...
    def handle_make_linkedin(self):
        ...
        if linkedin_post is None:
            ...
            result = llm.call(
                f"""
                You wrote this linkedin_post on {self.state.topic}, but it does not have a good virality score
                because of {self.state.score.reason}
